...
void run_ra_diff_m(int n, string s) {
    const string directory = "../../data/ra/deg_dist" + s + '/';
    //Make the directory if it doesn't exist
    string s1 = "mkdir ";
    string s2 = "../../data/ra/deg_dist";
    system((s1 + s2 + s).c_str());
    int m_array[6] = {1, 2, 4, 8, 16, 32};
    for (int i = 0; i < 6; i++) {
        simplegraph g = RAGraph(n, m_array[i]);
...
void run_ra_diff_n(int m, string s) {
    const string directory = "../../data/ra/deg_dist" + s + '/';
    //Make the directory if it doesn't exist
    string s1 = "mkdir ";
    string s2 = "../../data/ra/deg_dist";
    ...
    int n_array[6] = {100, 1000, 10000, 100000, 1000000, 10000000};
    for (int i = 0; i < 6; i++) {
        simplegraph g = RAGraph(n_array[i], m);
...
vector<int> generate_synthetic_ra(int m, int max, int N) {
    // Setup the random bits
    std::random_device rd;
    std::mt19937 gen(rd());
    vector<float> weights(max);
    for (int k = 0; k < max; k++) {
        if (k < m) {
            ...
            float a = 1. / (float(m) + 1) * (pow((float(m) / (float(m) + 1)), (float(k) - float(m))));
            weights[k] = a;
        }
    }
...
void generate_synthetic_ra_wrapper(int m, int max, int N) {
    const string directory = "../../data/ra/synthetic/";
    //Make the directory if it doesn't exist
    string s1 = "mkdir ";
    string s2 = "../../data/ba/synthetic";
    system((s1 + s2).c_str());
    ...
    for (int i=0; i < 25; i++) {
        ...
        oss << directory << N << "_" << m << "_" << i <<".csv";
        ofstream file (oss.str());
        vector<int> v = generate_synthetic_ra(m, max, N);
        std::copy(v.begin(), v.end(), std::ostream_iterator<int>(file, ","));
    }
}
